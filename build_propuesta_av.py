...
DOCX_PATH = OUT / "propuesta_estabilidad_video_iglesia.docx"
CURRENT_DIAGRAM = OUT / "diagrama_actual.png"
PROPOSED_DIAGRAM = OUT / "diagrama_propuesto.png"
...
def build_doc():
    ...
    header.text = "Propuesta AV | ProPresenter y proyectores"
    ...
    r = p.add_run("Uso con ProPresenter en iglesia pequeña | Monitor operador + salida principal + Stage Display")
    ...
    for label, value in [
        ...
        ("Fecha", "Junio de 2026"),
        ("Objetivo", "Reducir fallas de detección, reinicios y pérdida de señal en proyectores sin afectar Stage Display"),
    ]:
    ...
    add_paragraph(
        ...
        "HDBaseT está diseñado para llevar video HDMI a largas distancias de forma más estable. La TV del presentador debe mantenerse como salida independiente de la PC para Stage Display en ProPresenter; no debe conectarse al splitter porque recibiría la misma señal que los proyectores.",
    )

    add_heading(doc, "Objetivos de la mejora")
    for item in [
        "Que los dos proyectores reciban la misma señal principal de ProPresenter de forma estable.",
        ...
        "Evaluar el cambio de proyectores solo si el presupuesto lo permite o si los equipos actuales ya limitan brillo, resolución o confiabilidad.",
    ]:
        add_bullet(doc, item)
    ...
    add_paragraph(
        doc,
        "La propuesta cambia los tramos largos de la señal principal a Cat6A directo entre transmisor y receptores HDBaseT. El HDMI queda solamente en tramos cortos para proyectores. La TV del presentador se mantiene separada, conectada directamente a la PC como Stage Display.",
    )
    doc.add_picture(str(PROPOSED_DIAGRAM), width=Inches(6.55))
    for item in [
        "Configurar la salida de ProPresenter/Windows a 1920 x 1080, 60 Hz.",
        "Desactivar HDR y evitar resoluciones mixtas entre proyectores.",
    ...
    add_paragraph(
        doc,
        "Esta opción mejora la estabilidad de señal para los dos proyectores y también considera reemplazarlos por dos equipos iguales. La TV del presentador permanece en una salida separada de Stage Display desde la PC.",
    )
    rows_optimal = [
        ("Divisor HDMI 1x2 OREI UHD-PRO102 con EDID", "1", "$559.83", "$559.83", "Divide solo señal principal"),
        ("AV Access HDBaseT 4K60 / 1080p 230 ft", "2", "$2,747.88", "$5,495.76", "Un kit por proyector"),
        ...
        ("ViewSonic PA503HD 1080p 4000 lm", "2", "$12,999.00", "$25,998.00", "Cambio opcional de proyectores"),
    ]
    add_cost_table(
        doc,
        "Costos estimados - opción óptima",
        rows_optimal,
        "Subtotal con proyectores nuevos: $34,974.39 MXN. Si se conservan los proyectores actuales, el subtotal baja a aproximadamente $8,976.39 MXN. La instalación se contempla con voluntarios.",
    )

    add_heading(doc, "Por qué se sugieren esos proyectores", level=2)
    add_paragraph(
        doc,
        "El modelo ViewSonic PA503HD se eligió como referencia porque ofrece una combinación razonable para iglesia pequeña: resolución Full HD 1080p real, brillo anunciado de 4,000 lúmenes, entrada HDMI, marca conocida y precio menor que alternativas láser o modelos profesionales más grandes. "
        "La recomendación no depende de que sea exactamente ese modelo, pero sí de comprar dos proyectores iguales con características equivalentes.",
    )
    for item in [
        "Mejor que proyectores portátiles o genéricos: muchos anuncian números altos de lúmenes, pero no siempre son lúmenes ANSI reales ni están pensados para uso continuo en salón iluminado.",
        "Mejor consistencia visual: dos proyectores iguales facilitan igualar color, tamaño, brillo y resolución entre ambos lados.",
        "Mejor compatibilidad con ProPresenter: 1080p/60 Hz es una salida estándar, estable y suficiente para letras, fondos, videos y contenido de culto.",
        "Menor riesgo de mantenimiento: marcas conocidas suelen tener mejor soporte, repuestos, lámparas o documentación que equipos genéricos.",
        "Alternativas válidas: Epson Home Cinema 980 o BenQ TH575C también pueden funcionar; conviene elegir según precio, garantía, distancia de tiro y brillo real disponible.",
    ]:
        add_bullet(doc, item)
    ...
    comparisons = [
        ...
        ("Costo inicial", "Bajo/medio", "Medio/alto", "La óptima sube principalmente por el equipo HDBaseT 4K y proyectores nuevos."),
        ("Calidad visual", "Depende de equipos actuales", "Más consistente", "Dos proyectores iguales facilitan color y brillo."),
    ...
    add_paragraph(
        doc,
        "Aprobar primero la opción económica como Fase 1 para estabilizar la señal principal de los dos proyectores. Mantener la TV del presentador conectada directo a la PC como Stage Display. Si después de estabilizar la señal los proyectores actuales siguen mostrando diferencias fuertes de brillo/color, baja resolución o fallas por antigüedad, aprobar la Fase 2 para reemplazarlos por dos proyectores iguales.",
    )
    add_paragraph(
        doc,
    ...
    for item in [
        ...
        "Antes de instalar definitivamente, probar ambos proyectores durante al menos 30 minutos con ProPresenter enviando video y letras.",
        "Guardar una configuración fija en Windows/ProPresenter: 1080p, 60 Hz, salida duplicada solamente para proyectores.",
    ...
    sources = [
        ...
        ("ViewSonic PA503HD", "https://www.amazon.com.mx/dp/B0CJ4LMLY8"),
        ("Alternativa Epson Home Cinema 980", "https://www.amazon.com.mx/dp/B0FHT7MZW2"),
        ("Alternativa BenQ TH575C", "https://www.amazon.com.mx/dp/B0GY24YF22"),
    ]
